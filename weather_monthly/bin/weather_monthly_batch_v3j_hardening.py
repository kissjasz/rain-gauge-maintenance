...
#!/usr/bin/env python3
"""
weather_monthly_batch_v3j_hardening.py
- Broader fragment extraction: accept ANY updatePanel id (concat all payloads)
- Stronger fallbacks: try non-AJAX full postback if AJAX path yields no table
- Adds headers: Referer, X-Requested-With, proper Content-Type
- Saves raw/frag HTML at each step in --save-html
"""
...
def _extract_all_updatepanel_fragments(text: str) -> str:
    """Concatenate ALL updatePanel payloads from Microsoft AJAX delta."""
...
def _extract_show_all_target(html: str) -> str:
    # Prefer target near text Show All Records
    # First, try to locate the button block
    block = ""
    mbtn = re.search(r"(<button[^>]+__doPostBack\('[^']+',''\)[^>]*>.*?Show\s+All\s+Records.*?</button>)", html, flags=re.I|re.S)
    if mbtn:
        block = mbtn.group(1)
        m = re.search(r"__doPostBack\('([^']+)',''\)", block, flags=re.I)
        if m: return m.group(1)
    # Fallback: any __doPostBack target
    m = re.search(r"__doPostBack\('([^']+)',''\)", html, flags=re.I)
...
def fetch_daily(session,station_code,date_ddmmyyyy,time_hhmm,*,debug=False,save_html_dir=""):
    ...
    # If likely paged or empty, click Show All
    show_all_target = _extract_show_all_target(htmlB)
    if (not rows or len(rows) < 200) and show_all_target:
        if debug: print(f"[POST-C] Show All __EVENTTARGET={show_all_target}")
        ...
        if save_html_dir:
            with open(os.path.join(save_html_dir,f"{station_code}_{date_ddmmyyyy.replace('/','-')}_C_raw.txt"),"w",encoding="utf-8") as f: f.write(rawC)
            with open(os.path.join(save_html_dir,f"{station_code}_{date_ddmmyyyy.replace('/','-')}_C.html"),"w",encoding="utf-8") as f: f.write(htmlC)
        t2, tid2 = _find_result_table_in_html(htmlC)
        rows=_parse_table(t2) if t2 else rows
        if debug and rows: print(f"[DEBUG] After Show All: table={tid2} rows={len(rows)}")

    # If still nothing, try NON-AJAX full postback as last resort
    if not rows:
        if debug: print("[FALLBACK] Non-AJAX full postback")
        ...
        htmlN=rN.text
        if save_html_dir:
            with open(os.path.join(save_html_dir,f"{station_code}_{date_ddmmyyyy.replace('/','-')}_N.html"),"w",encoding="utf-8") as f: f.write(htmlN)
        tN, tidN = _find_result_table_in_html(htmlN)
        rows=_parse_table(tN) if tN else rows
        if debug and rows: print(f"[DEBUG] Non-AJAX: table={tidN} rows={len(rows)}")
...
def fetch_one_station_month(session,st,y,m,*,debug=False,save_html=False,outdir="weather_data"):
    ...
    cols=["station_code","station_name","date","_time","_rain_mm","_battery_v","_solar_v","_temp_c","_hum_pct"]
    for c in cols:
        if c not in df.columns: df[c] = ""
    df=df[cols]
    for c in ["_rain_mm","_battery_v","_solar_v","_temp_c","_hum_pct"]:
...
def main():
    ...
    if args.combined_csv or args.combined_excel:
        combo = pd.concat(combined, ignore_index=True) if combined else pd.DataFrame(columns=["station_code","station_name","date","_time","_rain_mm","_battery_v","_solar_v","_temp_c","_hum_pct","year","month"])

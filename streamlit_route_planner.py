...
def load_sheet_days() -> pd.DataFrame:
    """โหลดข้อมูลจาก Google Sheets พร้อม connection pooling"""
    ...
    try:
        ...
        ws = sh.worksheet(sa["SHEET_TAB"])
        values = ws.get(sa["SHEET_RANGE"])  # B..C
    except Exception as e:
        st.warning(f"ไม่สามารถโหลดข้อมูลจาก Google Sheets: {e}")
        return pd.DataFrame()
...
def smart_rerun():
    """Rerun with intelligent delay to prevent race conditions - ใช้เฉพาะเมื่อจำเป็น"""
    try:
        current_time = time.time()
        time_since_last = st.session_state.get('last_rerun_time', 0)

        if current_time - time_since_last < 0.5:  # ป้องกัน rapid rerun
            return  # ไม่ rerun ถ้ายังไม่ครบเวลา
...
def find_nearest_station_optimized(clicked_lat: float, clicked_lng: float, df: pd.DataFrame,
    ...
    """หาสถานีที่ใกล้ที่สุดอย่างมีประสิทธิภาพ - ใช้ vectorized operations พร้อม haversine"""
    try:
        min_distance = float('inf')
        closest_station = None

        # กรองข้อมูลที่มีพิกัดถูกต้อง
        valid_df = df[(df['lat'].notna()) & (df['lon'].notna()) &
                      (df['lat'] != 0.0) & (df['lon'] != 0.0)].copy()

        if valid_df.empty:
            return None

        # คำนวณระยะทางทั้งหมดพร้อมกัน (vectorized with haversine - เร็วกว่า geodesic 10-20 เท่า)
        try:
            lat_array = valid_df['lat'].to_numpy()
            lon_array = valid_df['lon'].to_numpy()
            distances = haversine_vectorized(clicked_lat, clicked_lng, lat_array, lon_array)

            # หาสถานีที่ใกล้ที่สุดภายในรัศมีที่กำหนด
            within_range_mask = distances < max_distance_m
            if within_range_mask.any():
                min_idx_in_array = np.argmin(distances[within_range_mask])
                # แปลง index กลับไปเป็น DataFrame index
                valid_indices = valid_df.index[within_range_mask].tolist()
                min_idx = valid_indices[min_idx_in_array]
                closest_station = str(valid_df.loc[min_idx, 'station_id'])
                min_distance = distances[within_range_mask][min_idx_in_array]
        except Exception:
            pass
...
def haversine_vectorized(lat1: float, lon1: float, lat2_array: np.ndarray, lon2_array: np.ndarray) -> np.ndarray:
    """คำนวณระยะทาง Haversine แบบ vectorized - เร็วกว่า geodesic 10-20 เท่า"""
...
def load_station_data(
    ...
    """โหลดสถานีจาก stations.json แล้วผสานชื่อไทย/URL จาก Latlonstation_config.json พร้อม memory optimization"""
    try:
        # --- load stations.json ---
        ...
        # ✅ ลด memory usage ด้วย categorical data types สำหรับคอลัมน์ที่มีค่าซ้ำ
...
# ✅ TSP Algorithm with improved caching
def _stations_list_to_hash(stations_data: List[Dict], start: str, end: str) -> str:
    """สร้าง hash key จากข้อมูลสถานีสำหรับ caching"""
    station_ids = sorted([s['station_id'] for s in stations_data])
    return f"{','.join(station_ids)}|{start}|{end}"
...
def calculate_optimal_route(
    ...
    """คำนวณเส้นทางที่เหมาะสมพร้อม performance optimization และ smart caching"""
        ...
        if len(selected_stations) >= 2:
            ...
            if st.button("🧮 คำนวณเส้นทางที่เหมาะสม"):
                if current_time - last_calc_time < 2:
                    ...
                    # ✅ Progress indicators สำหรับการคำนวณช้า
                    ...
                    try:
                        ...
                        status_text.text(f"🧮 กำลังคำนวณเส้นทางที่เหมาะสมสำหรับ {len(stations_data)} สถานี...")
                        ...
                        status_text.text("✨ กำลังเตรียมผลลัพธ์...")
